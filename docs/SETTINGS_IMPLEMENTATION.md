...
- **设置类型**: boolean
- **应实现功能**: 窗口激活时，自动聚焦搜索框
- **注意**: 可能与智能激活冲突，智能激活已包含聚焦搜索框
- **涉及文件**: `src/components/ClipboardList.vue`

#### 6. 自动粘贴 (auto_paste)
- **当前状态**: ⚠️ 部分实现（只读到了设置，没实现粘贴逻辑）
...
### 🔴 P0 - 核心体验（建议优先实现）
2. **auto_paste** - 自动粘贴行为（设置已读取，只需实现粘贴逻辑）
3. **window_position** - 窗口位置记忆
4. **search_position** - 搜索框位置

### 🟡 P1 - 增强体验
5. **copy_sound** - 复制音效
6. **confirm_delete** - 删除确认
7. **copy_as_plain_text** - 复制为纯文本
8. **auto_sort** - 自动排序

### 🟢 P2 - 进阶功能
9. **auto_start** - 开机自启
10. **paste_as_plain_text** - 粘贴为纯文本
11. **image_ocr** - 图片OCR
