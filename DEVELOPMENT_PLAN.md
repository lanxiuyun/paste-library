...
- [x] **3.3** 交互功能
  ...
  - ✅ 点击收藏 → 切换收藏状态
  - ✅ 点击删除 → 删除单条记录
  - ✅ 搜索框：底部固定，带 Ctrl+F 快捷键

- [x] **3.4** 窗口控制
  - [x] 顶部拖动手柄（简约细线设计，36px高度）
  - [x] 可调整窗口大小
  - [x] 快捷键集成 (Alt+V) — 已支持唤醒/隐藏剪贴板窗口
  - [x] 窗口失焦自动隐藏
...
- [x] **4.2** 设置面板
  ...
  - [x] 历史记录最大数量设置
  - [x] 开机自启设置（UI准备，功能待实现）
  - [x] 快捷键显示 (Alt+V，暂不支持自定义)
  - [x] 黑名单应用
  - [x] 自动清理周期
  - [x] 窗口位置设置 (记住位置/居中/跟随鼠标)
  - [x] 激活时回到顶部
  - [x] 激活时切换至全部分组
  - [x] 复制音效设置
  - [x] 搜索框位置 (顶部/底部)
...
- 2026-02-05: UI优化：搜索栏移至底部、拖拽手柄简化为细线、移除窗口控制按钮
- 2026-02-06: 实现全局快捷键(Alt+V)、窗口失焦自动隐藏、设置面板UI，重构为设置面板主窗口+剪贴板子窗口
- 2026-02-06: 添加完整设置项（窗口设置、音效设置、搜索设置、内容设置等），更新导航栏样式
